...
def do_train(cfg, model, resume=False):
    ...
    # gc.disable() # Memory Leak Bug: https://github.com/facebookresearch/dinov3/issues/150
    gc.collect()
    ...
    if start_iter == 0 and distributed.is_subgroup_main_process():
        shutil.copyfile("dinov3/train/ssl_meta_arch.py", os.path.join(
            cfg.train.output_dir, "ssl_meta_arch.py")
        )
        shutil.copyfile("dinov3/train/train.py", os.path.join(
            cfg.train.output_dir, "train.py")
        )
        shutil.copyfile("dinov3/loss/dino_clstoken_loss.py", os.path.join(
            cfg.train.output_dir, "dino_clstoken_loss.py")
        )
        shutil.copyfile("dinov3/loss/ibot_patch_loss.py", os.path.join(
            cfg.train.output_dir, "ibot_patch_loss.py")
        )
        shutil.copyfile("dinov3/layers/memory.py", os.path.join(
            cfg.train.output_dir, "memory.py")
        )
        shutil.copyfile("dinov3/layers/base.py", os.path.join(
            cfg.train.output_dir, "base.py")
        )
        shutil.copyfile("dinov3/layers/density.py", os.path.join(
            cfg.train.output_dir, "density_based.py")
        )
        shutil.copyfile("dinov3/layers/dino_head.py", os.path.join(
            cfg.train.output_dir, "dino_head.py")
        )
