...
from cam.checkpointer import init_fsdp_model_from_checkpoint
from cam.configs import get_default_config
from cam.data import DataAugmentationDINO
from cam.fsdp.ac_compile_parallelize import ac_compile_parallelize
from cam.layers.cam_head import CAMHead
from cam.loss import CAMLoss, GramLoss, KoLeoLoss, KoLeoLossDistributed, iBOTPatchLoss
from cam.models import build_model_from_cfg
from cam.train.cosine_lr_scheduler import linear_warmup_cosine_decay
from cam.train.param_groups import fuse_params_groups, get_params_groups_with_decay_fsdp
...
class CAMMetaArch(nn.Module):
    """
    Modified version of CAMMetaArchCompilable including gram loss:
    ...
    def __init__(self, cfg):
        ...
        self.class_memory = Memory(
                K=cfg.global_memory.memory_size,
                partition_size=cfg.global_memory.partition_size,
                num_tasks=cfg.global_memory.num_tasks,
                out_dim=cfg.global_memory.head_bottleneck_dim,

                # 1. CLOCK SPEED
                target_update_size=cfg.global_memory.target_update_size,

                # 2. THE POLICY (Pure LFU/LRU)
                priority_ratio=cfg.global_memory.priority_ratio,
                usage_decay=cfg.global_memory.usage_decay,
                
                # 3. REFINEMENT (Keep low to prevent Black Hole)
                refinement_ratio=cfg.global_memory.refinement_ratio,
                refinement_momentum=cfg.global_memory.refinement_momentum,

                # 4. THE GATEKEEPER
                threshold_initial=cfg.global_memory.threshold_initial,
                threshold_min=cfg.global_memory.threshold_initial - 0.2,
                threshold_max=0.99,
                refinement_update=cfg.global_memory.refinement_update,
                dedup_threshold=cfg.global_memory.dedup_threshold,
                grace_period_ratio=cfg.global_memory.grace_period_ratio,
                k_winners=cfg.global_memory.k_winners
            )

        # --- LOCAL MEMORY (LFU) ---
        # Goal: High turnover, but keep "sticky" local context features.
        self.patch_memory = Memory(
                K=cfg.local_memory.memory_size,
                partition_size=cfg.local_memory.partition_size,
                num_tasks=cfg.local_memory.num_tasks,
                out_dim=cfg.local_memory.head_bottleneck_dim,

                # 1. CLOCK SPEED
                target_update_size=cfg.local_memory.target_update_size,

                # 2. THE POLICY (Pure LFU)
                priority_ratio=cfg.local_memory.priority_ratio,
                usage_decay=cfg.local_memory.usage_decay,

                # 3. REFINEMENT
                refinement_ratio=cfg.local_memory.refinement_ratio,
                refinement_momentum=cfg.local_memory.refinement_momentum,

                # 4. THE GATEKEEPER
                threshold_initial=cfg.local_memory.threshold_initial,
                threshold_min=cfg.local_memory.threshold_initial - 0.2,
                threshold_max=0.99,
                refinement_update=cfg.local_memory.refinement_update,
                dedup_threshold=cfg.local_memory.dedup_threshold,
                grace_period_ratio=cfg.local_memory.grace_period_ratio,
                k_winners=cfg.local_memory.k_winners
            )
        ...
        student_model_dict["cam_head"] = cam_head_class()
        teacher_model_dict["cam_head"] = cam_head_class()
        self.dino_loss = CAMLoss(self.dino_out_dim)
    ...
    def _setup_distillation(self):
        ...
        teacher_model_dict["cam_head"] = CAMHead(
            in_dim=embed_dim,
        ...
        teacher_model_dict["ibot_head"] = CAMHead(
            in_dim=embed_dim,
    ...
    def init_weights(self) -> None:
        ...
        self.student.cam_head.init_weights()
        self.student.ibot_head.init_weights()
        self.dino_loss.init_weights()
        self.ibot_patch_loss.init_weights()

        self.class_memory.init_weights()
        self.patch_memory.init_weights()

        device = next(self.student.parameters()).device
        self.class_memory.to(device)
        self.patch_memory.to(device)
        ...
        if self.cfg.distillation.enabled:
            if self.cfg.distillation.checkpoint_path != "ignore":
                ...
                self.teacher.cam_head.init_weights()
    ...
    def forward_backward(
        ...
        image_targets = data["class_targets"].cuda(non_blocking=True)
        ...
        # teacher_global['cls_after_head'] torch.Size([2, 256, 256])
        # teacher_global['masked_patch_after_head'] torch.Size([14965, 256])
        # print("teacher_global['cls_after_head']", teacher_global["cls_after_head"].shape)
        # print("teacher_global['masked_patch_after_head']", teacher_global["masked_patch_after_head"].shape)


        # student_global['cls_after_head'] torch.Size([2, 256, 256])
        # student_global['masked_patch_after_head'] torch.Size([14965, 256])
        # student_local['cls_after_head'] torch.Size([8, 256, 256])
        ...
        # print("student_global['cls_after_head']", student_global["cls_after_head"].shape)
        # print("student_global['masked_patch_after_head']", student_global["masked_patch_after_head"].shape)
        # print("student_local['cls_after_head']", student_local["cls_after_head"].shape)
    ...
    def get_teacher_output(
        ...
        *,
        ...
        cls_after_head = self.teacher.cam_head(cls)  # [n_crops * B, K]
    ...
    def get_student_output(self, *, global_crops, local_crops, upperbound, masks, mask_indices_list):
        ...
        buffer = self.student.cam_head(buffer)  # [n_global_crops * B + n_local_crops * B, K]
    ...
    def compute_losses(
        ...
        *,
        ...
        # print("---- (local) self.dino_loss expected shapes ----")
        # print("""student_local["cls_after_head"]:""", student_local["cls_after_head"].shape)
        # print("""teacher_global["cls_centered"]:""", teacher_global["cls_centered"].shape)
        # print("----------")
        # ---- (local) self.dino_loss expected shapes ----
        # student_local["cls_after_head"]: torch.Size([8, 256, 65536])
        # teacher_global["cls_centered"]: torch.Size([2, 256, 65536])
        # ----------

        with torch.amp.autocast("cuda", dtype=torch.bfloat16):
            student_local_cls_after_memory_list, teacher_dino_cls_after_memory_list = self.class_memory.forward_global(
                student_local['cls_after_head'], teacher_global["cls_after_head"], student_temp=0.1, teacher_temp=teacher_temp, update_memory=False)

        dino_local_crops_loss = 0
        for student_local_cls_after_memory, teacher_dino_cls_after_memory in zip(student_local_cls_after_memory_list, teacher_dino_cls_after_memory_list):
            # print("student_local_cls_after_memory:", student_local_cls_after_memory.shape)
            # print("teacher_dino_cls_after_memory:", teacher_dino_cls_after_memory.shape)
            local_crops_loss = self.dino_loss(
                student_logits=student_local_cls_after_memory,
                teacher_probs=teacher_dino_cls_after_memory,
            )
            dino_local_crops_loss += local_crops_loss
        
        dino_local_crops_loss /= len(student_local_cls_after_memory_list)
        ...
        # print("---- (global) self.dino_loss expected shapes ----")
        # print("""student_global["cls_after_head"]:""", student_global["cls_after_head"].shape)
        # print("""teacher_global["cls_centered"]:""", teacher_global["cls_centered"].shape)
        # print("----------")
        # ---- (global) self.dino_loss expected shapes ----
        # student_global["cls_after_head"]: torch.Size([2, 256, 65536])
        # teacher_global["cls_centered"]: torch.Size([2, 256, 65536])
        # ----------

        with torch.amp.autocast("cuda", dtype=torch.bfloat16):
            student_global_cls_after_memory_list, teacher_dino_cls_after_memory_list = self.class_memory.forward_global(
                student_global['cls_after_head'], teacher_global["cls_after_head"], student_temp=0.1, teacher_temp=teacher_temp,
                update_memory=True, true_labels=image_targets)

        dino_global_crops_loss = 0
        for student_global_cls_after_memory, teacher_dino_cls_after_memory in zip(student_global_cls_after_memory_list, teacher_dino_cls_after_memory_list):
            # print("student_global_cls_after_memory:", student_global_cls_after_memory.shape)
            # print("teacher_dino_cls_after_memory", teacher_dino_cls_after_memory.shape)
            global_crops_loss = self.dino_loss(
                student_logits=student_global_cls_after_memory,
                teacher_probs=teacher_dino_cls_after_memory,
                ignore_diagonal=self.dino_global_ignore_diagonal,
            )
            dino_global_crops_loss += global_crops_loss

        dino_global_crops_loss /= len(student_global_cls_after_memory_list)
        ...
        # print("---- self.ibot_patch_loss expected shapes ----")
        # print("""student_global["masked_patch_after_head"]:""", student_global["masked_patch_after_head"].shape)
        # print("""teacher_global["masked_patch_centered"]:""", teacher_global["masked_patch_centered"].shape)
        # print("masks:", masks.shape)
        # print("----------")
        # ---- self.ibot_patch_loss expected shapes ----
        # student_global["masked_patch_after_head"]: torch.Size([14965, 65536])
        # teacher_global["masked_patch_centered"]: torch.Size([14965, 65536])
        # masks: torch.Size([512, 196])
        # ----------

        # assert student_global["masked_patch_after_head"].shape == teacher_global["masked_patch_after_head"].shape
        with torch.amp.autocast("cuda", dtype=torch.bfloat16):
            student_global_masked_patch_after_memory_list, masked_teacher_ibot_after_memory_list = self.patch_memory.forward_local(
                student_global["masked_patch_after_head"], teacher_global["masked_patch_after_head"], student_temp=0.1, teacher_temp=teacher_temp)

        ibot_patch_loss = 0
        for student_global_masked_patch_after_memory, masked_teacher_ibot_after_memory in zip(student_global_masked_patch_after_memory_list, masked_teacher_ibot_after_memory_list):
            # print("student_global_masked_patch_after_memory:", student_global_masked_patch_after_memory.shape)
            # print("masked_teacher_ibot_after_memory", masked_teacher_ibot_after_memory.shape)
            # IBOT loss
            patch_loss = self.ibot_patch_loss.forward_masked(
                student_global_masked_patch_after_memory,
                masked_teacher_ibot_after_memory,
                student_masks_flat=masks,
                n_masked_patches=mask_indices_list.shape[0],
                masks_weight=masks_weight,
            )
            ibot_patch_loss += patch_loss

        ibot_patch_loss /= len(student_global_masked_patch_after_memory_list)
    ...
    def gram_load_ema_teacher(self):
        if self.has_gram_teacher:
            skip_load_prefixes = ["cam_head.", "ibot_head."]
    ...
    def get_maybe_fused_params_for_submodel(self, m: nn.Module):
        params_groups = get_params_groups_with_decay_fsdp(
            ...
            cam_head_wd_multiplier=self.cfg.optim.cam_head_wd_multiplier,
        )
