...
from cam.configs import setup_config, setup_job, setup_multidistillation
from cam.data import (
    MaskingGenerator,
    SamplerType,
...
from cam.logging import MetricLogger, setup_logging
from cam.train.cosine_lr_scheduler import CosineScheduler, linear_warmup_cosine_decay
from cam.train.multidist_meta_arch import MultiDistillationMetaArch
from cam.train.cam_meta_arch import CAMMetaArch
...
def do_train(cfg, model, resume=False):
    ...
    # gc.disable() # Memory Leak Bug: https://github.com/facebookresearch/cam/issues/150
    gc.collect()
    ...
    if start_iter == 0 and distributed.is_subgroup_main_process():
        shutil.copyfile("cam/train/cam_meta_arch.py", os.path.join(
            cfg.train.output_dir, "cam_meta_arch.py")
        )
        shutil.copyfile("cam/train/train.py", os.path.join(
            cfg.train.output_dir, "train.py")
        )
        shutil.copyfile("cam/loss/cam_loss.py", os.path.join(
            cfg.train.output_dir, "cam_loss.py")
        )
        shutil.copyfile("cam/loss/ibot_patch_loss.py", os.path.join(
            cfg.train.output_dir, "ibot_patch_loss.py")
        )
        shutil.copyfile("cam/layers/memory.py", os.path.join(
            cfg.train.output_dir, "memory.py")
        )
        shutil.copyfile("cam/layers/base.py", os.path.join(
            cfg.train.output_dir, "base.py")
        )
        shutil.copyfile("cam/layers/density.py", os.path.join(
            cfg.train.output_dir, "density_based.py")
        )
        shutil.copyfile("cam/layers/cam_head.py", os.path.join(
            cfg.train.output_dir, "cam_head.py")
        )
    ...
    for data in metric_logger.log_every(
        ...
        if iteration % OFFICIAL_EPOCH_LENGTH == 0:
            model.class_memory.show_memory_healthy_report()
            model.patch_memory.show_memory_healthy_report()
        ...
        if (iteration + 1) % 150 == 0:
            ...
            gc.collect()
            try:
                # RSS = Resident Set Size (RAM currently occupied by this process)
                mem_usage_gb = psutil.Process(os.getpid()).memory_info().rss / (1024 ** 3)
                sys_mem_percent = psutil.virtual_memory().percent
                logger.info(f"[Rank {distributed.get_rank()}] CPU RAM Used: {mem_usage_gb:.2f} GB | System Total: {sys_mem_percent}%")
            except Exception:
                pass
    ...
    # --- SAVE MEMORY EMBEDDINGS AS NUMPY ARRAYS ---
    # Only save on the main process to prevent file conflicts
    if distributed.is_subgroup_main_process():

        logger.info("Saving memory state to disk...")
        try:

            # Helper to save tensor as npy
            def save_buffer(buffer, filename):
                arr = buffer.detach().cpu().numpy()
                path = os.path.join(cfg.train.output_dir, filename)
                np.save(path, arr)
                return path

            # 1. Global (Class) Memory
            save_buffer(model.class_memory.queue, "global_memory_embeddings.npy")
            save_buffer(model.class_memory.queue_usage, "global_memory_usage.npy")
            save_buffer(model.class_memory.queue_label, "global_queue_label.npy")
            save_buffer(model.class_memory.queue_age, "global_memory_age.npy")
            save_buffer(model.class_memory.queue_last_hit_age, "global_memory_last_hit_age.npy")
            logger.info(f"Saved Global Memory buffers to {cfg.train.output_dir}")

            # 2. Local (Patch) Memory
            save_buffer(model.patch_memory.queue, "local_memory_embeddings.npy")
            save_buffer(model.patch_memory.queue_usage, "local_memory_usage.npy")
            save_buffer(model.patch_memory.queue_age, "local_memory_age.npy")
            save_buffer(model.patch_memory.queue_last_hit_age, "local_memory_last_hit_age.npy")
            logger.info(f"Saved Local Memory buffers to {cfg.train.output_dir}")
