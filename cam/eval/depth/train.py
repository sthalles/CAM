...
from cam.eval.depth.data import build_dataloader
from cam.eval.depth.config import (
    DepthConfig,
    ResultConfig,
...
from cam.eval.depth.checkpoint_utils import find_latest_checkpoint, load_checkpoint, save_checkpoint
from cam.eval.depth.datasets.datasets_utils import _EvalCropType, make_valid_mask
from cam.eval.depth.loss import MultiLoss
from cam.eval.depth.models import Depther, make_depther_from_config
from cam.eval.depth.metrics import DEPTH_METRICS
from cam.eval.depth.schedulers import build_scheduler
from cam.eval.depth.eval import evaluate_depther_with_dataloader

from cam.eval.depth.utils import setup_model_ddp
...
class IterBasedTrainer:
    ...
    def train_on_batch(self, batch):
        ...
        target_dtype = next(self.depther.parameters()).dtype
        device = self.rank
        batch_img, depth_gt = batch
        batch_img = batch_img.to(device=device, dtype=target_dtype, non_blocking=True)
...
def run_epochs(config: DepthConfig, backbone: torch.nn.Module, autocast_dtype: torch.dtype):
    ...
    if config.load_from is not None:
        ...
        # Check if the model is wrapped in DDP and load accordingly
        if hasattr(depther.decoder, 'module'):
            depther.decoder.module.load_state_dict(state_dict)
        else:
            depther.decoder.load_state_dict(state_dict)
        
        if "optimizer" in chkpt:
            optimizer.load_state_dict(chkpt["optimizer"])
        else:
            # Optional: print a warning so you know it was skipped
            print("Warning: 'optimizer' key not found in checkpoint. Skipping optimizer state load.")
